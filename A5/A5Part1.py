...
def minFreqEstErr(inputFile, f):
    ...
    for k in range(1, 100):
        ...
        fftbuffer = x[x.size // 2 - hM2:x.size // 2 + hM1]
        N = Ns[np.where(Ns > M)[0][0]]
        mX, pX = DFT.dftAnal(fftbuffer, w, N)
        peak_loc = UF.peakDetection(mX, t)
        i_peak_loc = UF.peakInterp(mX, pX, peak_loc)[0]
        fEst = fs * i_peak_loc[0] / N
        if abs(fEst - f) <= error:
            break

    return (fEst, M, N)

#print(minFreqEstErr('sine-200.wav', 200))
